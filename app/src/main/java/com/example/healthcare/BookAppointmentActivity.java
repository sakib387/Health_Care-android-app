    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnbook.setOnClickListener(new View.OnClickListener() {
           ...
           try {
               ...
               if (db.checkAppointment(username, title + " => " + fullname, address, userphone, dateButton.getText().toString(), TimeButton.getText().toString()) == 1) {
                   Toast.makeText(getApplicationContext(), "Appoinctment already booked", Toast.LENGTH_SHORT).show();
               } else {
                   db.addOrder(username, title + " => " + fullname, address, userphone, 0, dateButton.getText().toString(), TimeButton.getText().toString(), Float.parseFloat(fees), "appointment");
                   Toast.makeText(getApplicationContext(), "Your Appoinctment is done successfully", Toast.LENGTH_SHORT).show();
                   startActivity(new Intent(BookAppointmentActivity.this, HomeActivity.class));
               }
           }
           catch (Exception e){
               Toast.makeText(getApplicationContext(), e.getMessage()+"You wrong", Toast.LENGTH_SHORT).show();
